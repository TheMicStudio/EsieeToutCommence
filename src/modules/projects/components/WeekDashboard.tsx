...
import {
  Users, BookOpen, CalendarClock, ClipboardList,
...
export function WeekDashboard({
  ...
  return (
    <div className="space-y-10">
      ...
      {!isProf && myGroup && (
        <div className="flex items-center gap-3 rounded-2xl bg-gradient-to-r from-[#0471a6]/5 to-[#89aae6]/5 border border-[#0471a6]/15 px-5 py-3.5">
          ...
          {myGroup.note !== undefined && myGroup.note !== null && (
            <div className="flex items-center gap-1.5 rounded-xl bg-[#0471a6]/10 px-3 py-1 ml-auto">
      ...
      {/* ── Section Groupes ─────────────────────────────────── */}
      <section>
        <SectionHeader icon={Users} label="Groupes" count={groups.length} />

        {!isProf && !myGroup && (
          <div className="flex items-center gap-3 rounded-xl border border-dashed border-[#89aae6]/50 bg-[#89aae6]/5 px-4 py-3 mb-4">
                ...
                >
                  ...
                  <div className="px-4 py-4 space-y-3">
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      {Array.from({ length: Math.min(spots, 3) }).map((_, i) => (
      ...
      {/* ── Section Supports ────────────────────────────────── */}
      <section>
        <SectionHeader icon={BookOpen} label="Supports de cours" count={materials.length || undefined} />
        <WeekCourseMaterialsPanel weekId={weekId} materials={materials} isProf={isProf} />
      </section>

      {/* ── Section Créneaux ────────────────────────────────── */}
      <section>
        <SectionHeader icon={CalendarClock} label="Créneaux de soutenance" count={slots.length || undefined} />
        {isProf && (
          <div className="mb-5">
      ...
      {/* ── Section Rétro ───────────────────────────────────── */}
      <section>
        <SectionHeader icon={ClipboardList} label="Rétrospective" />
        {retroBoard ? (
          <RetroBoard
            board={retroBoard}
          ...
          <div className="flex h-28 items-center justify-center rounded-2xl border border-dashed border-slate-200">
