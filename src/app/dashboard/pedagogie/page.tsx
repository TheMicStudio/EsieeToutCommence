...
export default async function PedagogiePage() {
  ...
  // Coordinateur / Admin → toutes les classes
  const allClasses = isCoord ? await getAllClasses() : [];
  // Professeur → ses classes assignées
  const teacherClasses = isProf ? await getMyTeacherClasses() : [];
  // Élève → ses classes (courante + passées)
  const allMyClasses = !isProf && !isCoord ? await getMyAllClasses() : [];
  ...
  if (!isProf && !isCoord && currentClass && pastClasses.length === 0) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Liste des classes (coordinateur — toutes les classes) */}
      {isCoord && allClasses.length > 0 && (
      ...
      {/* Alerte coordinateur sans aucune classe créée */}
      {isCoord && allClasses.length === 0 && (
        <div className="rounded-2xl border border-amber-200/70 bg-amber-50 p-5">
          <p className="text-sm font-semibold text-amber-800">Aucune classe créée</p>
          <p className="mt-1 text-xs text-amber-600">Créez des classes depuis le panneau Administration.</p>
        </div>
      )}
